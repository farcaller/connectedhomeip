...
class DeviceEnergyManagementClusterSlotStruct(
  ...
  val manufacturerESAState: Optional<Int>,
  ...
  val minDurationAdjustment: Optional<Long>,
  val maxDurationAdjustment: Optional<Long>
) {
  ...
  companion object {
    ...
    fun fromTlv(tag: Tag, tlvReader: TlvReader): DeviceEnergyManagementClusterSlotStruct {
      ...
      val manufacturerESAState =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MANUFACTURER_E_S_A_STATE))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_MANUFACTURER_E_S_A_STATE)))
        } else {
          Optional.empty()
        }
      val nominalPower =
      ...
      val minDurationAdjustment =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MIN_DURATION_ADJUSTMENT))) {
          Optional.of(tlvReader.getLong(ContextSpecificTag(TAG_MIN_DURATION_ADJUSTMENT)))
        } else {
          Optional.empty()
        }
      val maxDurationAdjustment =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MAX_DURATION_ADJUSTMENT))) {
          Optional.of(tlvReader.getLong(ContextSpecificTag(TAG_MAX_DURATION_ADJUSTMENT)))
        } else {
          Optional.empty()
        }
