...
class EnergyEvseClusterFaultEvent(
  val sessionId: Long,
  val state: Int,
  val faultStatePreviousState: Int,
  val faultStateCurrentState: Int
) {
  ...
  companion object {
    ...
    fun fromTlv(tag: Tag, tlvReader: TlvReader): EnergyEvseClusterFaultEvent {
      ...
      val faultStatePreviousState =
        tlvReader.getInt(ContextSpecificTag(TAG_FAULT_STATE_PREVIOUS_STATE))
      val faultStateCurrentState =
        tlvReader.getInt(ContextSpecificTag(TAG_FAULT_STATE_CURRENT_STATE))
