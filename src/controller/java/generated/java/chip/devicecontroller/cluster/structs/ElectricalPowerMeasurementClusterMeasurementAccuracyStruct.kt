...
class ElectricalPowerMeasurementClusterMeasurementAccuracyStruct(
  ...
  val percentTypical: Optional<Int>,
  val percentMin: Optional<Int>,
  val percentMax: Optional<Int>,
    ...
    fun fromTlv(
      ...
      val percentTypical =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_PERCENT_TYPICAL))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_PERCENT_TYPICAL)))
        } else {
          Optional.empty()
        }
      val percentMin =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_PERCENT_MIN))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_PERCENT_MIN)))
        } else {
          Optional.empty()
        }
      val percentMax =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_PERCENT_MAX))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_PERCENT_MAX)))
        } else {
          Optional.empty()
        }
      val fixedTypical =
