                                       ...
                                       uint8_t * value)
{
    ...
    {
        WaterHeaterMgr().InitiateAction(*value ? WaterHeaterManager::ON_ACTION : WaterHeaterManager::OFF_ACTION);
    }
}
...
int main(int argc, char * argv[])
{
    ...
    CHIP_ERROR err = WaterHeaterMgr().Init();
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(AppServer, "Failed to initialize water heater manager: %" CHIP_ERROR_FORMAT, err.Format());
